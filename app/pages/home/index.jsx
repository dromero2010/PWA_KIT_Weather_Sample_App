...
const NewHeader = () => {
    ...
    const {isLoading, error, data} = useQuery({
        ...
        queryFn: () =>
            fetch(
                //must add api key from openweathermap.org or an error will be returned
                `https://api.openweathermap.org/data/2.5/forecast?lat=41.88&lon=-87.62&units=imperial&appid=${api}`
            ).then((res) => res.json())
    })
    ...
    return (
        <Box maxW="100%" borderWidth="2px" bgColor="white" align="center">
            <Stack direction="column">
                <Box>
                    <Text fontSize="5xl">Weather in {data.city.name}</Text>
                </Box>
                <Box>
                    <Text fontSize="2xl">
                        Date: {moment(data.list[0].dt_txt).format('MMMM Do, YYYY')}
                    </Text>
                    <Text>Temperature: {data.list[0].main.temp}°F</Text>
                    <Text>Weather: {data.list[0].weather[0].description}</Text>
                    <Text>Sunrise: {moment.unix(data.city.sunrise).format('LT')}</Text>
                    <Text>Sunset: {moment.unix(data.city.sunset).format('LT')}</Text>
                </Box>
            </Stack>
        </Box>
    )
